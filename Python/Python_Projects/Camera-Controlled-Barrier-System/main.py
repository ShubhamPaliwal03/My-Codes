...
def isRegisteredVehicle(vehicleNumber): #
    ...
    for v_num in authorized_vehicles_data:

        if vehicleNumber in v_num:
...
def filter_numberplate_string(recognized_string: str) -> str:

    # Use a list comprehension to filter out unwanted characters

    filtered = ''.join([char for char in recognized_string if char.isdigit() or char.isupper()])
...
def sendDataToArduino():
    ...
    bt_port_outgoing = 'COM4' # Replace with the correct ouotgoing port for your Bluetooth module
    ...
    try:
        ...
        ser_out = serial.Serial(bt_port_outgoing, 9600, timeout=1)

        print(f"Connected to HC-05 on outgoing serial port {bt_port_outgoing}!")
        ...
        if ser_out is not None:
            ...
            ser_out.close()
            print(f"Outgoing serial port {bt_port_outgoing} closed...")

            return True

        else:

            print(f"Outgoing serial port {bt_port_outgoing} Not Opened!")

            return False

    except (serial.SerialException, OSError) as e:

        print(f"Failed to communicate with HC-05 on outgoing port {bt_port_outgoing}: {e}")

        return False

def closeCaptureAndDestroyWindows():
...
if __name__ == "__main__":
    ...
    # load Haar Cascade and EasyOCR Reader (only once)
    ...
    while True:
        ...
        for (x, y, w, h) in plates:
            ...
            if area > min_area:
                ...
                # Perform OCR directly on the ROI (without saving and loading the image, to ensure optimal performance)

                plate_number = filter_numberplate_string(getPlateNumberOCR(img_roi).replace(" ", ""))
                print(plate_number)

                if len(plate_number) >= 9 and plate_number != "NOT FOUND":
                ...
                if isMatched:
                    ...
                    isDataSuccessfullySentToArduino = sendDataToArduino()
                    ...
                    if isDataSuccessfullySentToArduino:
                        ...
                        print("Problem sending barrier release command to barrier control system...Try Again...If problem persists, troubleshoot manually!")
                        print(f"Data Entry of vehicle {v_num} not done in the vehicle entry records!")

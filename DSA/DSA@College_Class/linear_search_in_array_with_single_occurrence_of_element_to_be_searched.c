...
int main()
{
	...
	for(i=0;i<n;i++)
	{
		if(arr[i]==key)
		{
			printf("\nRequired element %d found in the array entered at index number %d or position %d",key,i,i+1);
			break;
			/*to prevent loop for running over again and again as per the condition and to 
			avoid multiple statements enclosed in printf statement being shown in output*/
		}
	}
	if(i==n)
	...
	{
		printf("nRequired element %d not found in the array entered",key);
	}
	return 0;
}